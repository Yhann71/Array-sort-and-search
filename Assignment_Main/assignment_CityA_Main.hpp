...
node* readCSV(const string& filename)
{
    ...
    while (getline(file, line))
    {
        ...
        getline(ss, newNode->data.ResidentID,        ',');
        ss >> newNode->data.Age;      
        ss.ignore(); // ignore comma
        getline(ss, newNode->data.ModeOfTransport,   ',');
        ss >> newNode->data.DailyDistance;
        ss.ignore();
        ss >> newNode->data.CarbonEmissionFactor;
        ss.ignore();
        ss >> newNode->data.AverageDaysPerMonth;
        ss.ignore();
...
// returns 1=Age, 2=Mode of Transport, 3=Daily Distance Threshold
...
void displaySortResults(node* group, int groupOpt, int n, string sortName, double timeTaken)
{
    ...
    // --- Memory Usage ---
    cout << "\n--- Memory Usage ---\n";
    cout << "Memory Usage : " << n << " elements / " << sizeof(CityA) << " bytes per element = " << (n * sizeof(CityA)) << " bytes\n";
...
{
    int outAge = -1, outDistance = -1;
    string outTransport = "";
    int field = searchByMenu(outAge, outTransport, outDistance);
    ...
    {
        bool match = false;
        if      (field == 1 && curr->data.Age             == outAge)       match = true;
...
void binarySearchUser(node* head)
{
    int outAge = -1, outDistance = -1;
    string outTransport = "";
    // shared menu: 1=Age, 2=Transport, 3=Distance
    int field = searchByMenu(outAge, outTransport, outDistance);
    if (field == -1) { dataset1CityA(); return; }
    ...
    if (field == 1) // Binary search by Age
    {
        ...
        while (lo <= hi)
        {
            ...
            if      (midNode->data.Age == outAge) { found = mid; hi = mid - 1; }
            else if (midNode->data.Age <  outAge)   lo = mid + 1;
            else                                    hi = mid - 1;
        }
        if (found != -1)
        {
            ...
            while (n2 != nullptr && n2->data.Age == outAge)
            {
    ...
    {
        // List sorted ascending by Distance — find first node > threshold
        int lo = 0, hi = n - 1, found = -1;
        while (lo <= hi)
        {
            int mid = lo + (hi - lo) / 2;
            ...
            {
                found = mid;    // qualifies, try to find earlier one
                hi = mid - 1;
            }
            else
            {
                lo = mid + 1;   // does not qualify, search right
            }
        }
        if (found != -1)
        {
